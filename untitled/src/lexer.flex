...
// To accept hex
[0-9]+[xX][0-9a-fA-F]+              {return new token("NUMBER", yytext(), yyline);}

// To accept integers with exp
[0-9]+([eE][+-]?[0-9]+)?            {return new token("NUMBER", yytext(), yyline);}

// To accept decimals with exp
[0-9]+\.[0-9]+([eE][+-]?[0-9]+)?    {return new token("NUMBER", yytext(), yyline);}
